...
export default function ChatLauncher() {
  ...
  const currentPropertyId = pathname.startsWith("/properties/")
    ? pathname.split("/properties/")[1]?.split(/[?#/]/)[0]
    : null;

  const currentSuggestions = currentPropertyId
    ? ["Review nhanh qua căn này", "Pháp lý căn này thế nào?", "Đặt lịch xem căn này"]
    : suggestions;

  const [messages, setMessages] = useState<ChatItem[]>([
    {
      role: "assistant",
      content: currentPropertyId
        ? "Xin chào! Tôi có thể hỗ trợ review chi tiết căn nhà bạn đang xem, giải đáp pháp lý hoặc đặt lịch xem nhà ngay."
        : "Xin chào! Tôi có thể giúp bạn tìm nhà, lọc theo ngân sách và đặt lịch xem.",
    },
  ]);
  ...
  return <div className="fixed bottom-5 right-5 z-50">
    {open && <section className="mb-3 flex h-[min(620px,calc(100vh-110px))] w-[min(390px,calc(100vw-32px))] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl" aria-label="Trợ lý tìm nhà">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={message.role === "user" ? "flex justify-end" : "flex justify-start"}><div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "rounded-br-sm bg-[#0b132b] text-white" : "rounded-bl-sm border border-slate-100 bg-white text-slate-700 shadow-sm"}`}><div className="text-sm leading-relaxed [&>p]:mb-2 [&>ul]:mb-2 [&>ul]:list-disc [&>ul]:pl-4 [&>ol]:list-decimal [&>ol]:pl-4 [&>strong]:font-semibold [&>h3]:text-sm [&>h3]:font-bold [&>h3]:mb-1"><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown></div>{message.properties?.map((property) => { const imgUrl = property.image || property.media?.[0]?.url; return <article key={property.id} className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="h-28 bg-slate-100">{imgUrl ? <PropertyImage src={imgUrl} alt={property.title} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-3xl">🏠</div>}</div><div className="p-3"><p className="line-clamp-2 text-xs font-bold text-slate-800">{property.title}</p><p className="mt-1 text-sm font-bold text-teal-700">{displayPrice(property.list_price)}</p><div className="mt-2 flex gap-2"><Link href={`/properties/${property.id}`} onClick={() => setOpen(false)} className="flex-1 rounded-lg border border-slate-200 py-1.5 text-center text-xs font-semibold">Chi tiết</Link><Link href={`/booking/schedule?property_id=${property.id}`} onClick={() => setOpen(false)} className="flex-1 rounded-lg bg-teal-600 py-1.5 text-center text-xs font-semibold text-white">Đặt lịch</Link></div></div></article>})}</div></div>)}{loading && <div className="flex items-center gap-2 text-xs text-slate-500"><span className="h-2 w-2 animate-pulse rounded-full bg-teal-500" />AI đang phân tích...</div>}</div>
      <div className="border-t border-slate-100 bg-white p-3"><div className="mb-2 flex gap-2 overflow-x-auto">{currentSuggestions.map((suggestion) => <button key={suggestion} onClick={() => void send(undefined, suggestion)} className="whitespace-nowrap rounded-full border border-teal-100 bg-teal-50 px-3 py-1.5 text-[11px] font-semibold text-teal-700">{suggestion}</button>)}</div><form onSubmit={(event) => void send(event)} className="flex gap-2"><input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Hỏi Nera về căn nhà này..." className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-teal-500" aria-label="Tin nhắn cho trợ lý" /><button disabled={loading || !input.trim()} className="rounded-xl bg-[#0b132b] px-4 text-white disabled:opacity-40" aria-label="Gửi"><FaPaperPlane /></button></form></div>
    </section>}
