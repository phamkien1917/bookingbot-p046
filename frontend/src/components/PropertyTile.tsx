    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.65rem] border border-black/5 bg-white shadow-[0_12px_40px_rgba(22,47,42,.07)] transition duration-500 hover:-translate-y-1.5 hover:shadow-[0_28px_70px_rgba(22,47,42,.14)]">
      <div className={`relative overflow-hidden bg-stone-100 ${compact ? "h-44" : "h-56"}`}>
        <Link href={`/properties/${property.id}`} className="block h-full">
          {image ? <PropertyImage src={image} alt={displayTitle} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /> : <div className="grid h-full place-items-center text-5xl">🏡</div>}
        </Link>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-4">
          <Link href={`/properties/${property.id}`} className="line-clamp-2 font-semibold leading-6 transition hover:text-[var(--forest)]">{displayTitle}</Link>
