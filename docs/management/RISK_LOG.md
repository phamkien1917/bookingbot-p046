...
| R-011 | Backend branch (`feature/backend-phamkien`) triển khai lại Sale/Booking/Admin routes và Redis orchestration đi ngược D-010 | Branch lỗi import và thiếu cấu hình, không khởi động được; Gate 2 đã bỏ qua nhánh này và dùng `develop` làm nền — xem `docs/demo/GATE2_REPORT.md` mục 3, quyết định D-013 | Cao | Cao | Product/PM + Project Lead + Backend/Data Owner (Phạm Kiên) | Không merge branch vào `main`/`develop` trước khi có buổi align riêng với Backend/Data Owner; đối chiếu scope backend với `MVP_SCOPE.md`/D-010 | Branch được mở PR/merge vào `main` hoặc `develop` trước khi có review | Mitigated |
